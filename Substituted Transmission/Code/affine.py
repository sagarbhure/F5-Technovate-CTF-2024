...
def encode():
    a = input('What co-prime number do you want to use for a? ')
    while(gcd(int(a), 26) != 1):
        a = input('Sorry, that number is not co-prime with 26. Please enter a different number: ')
...
def decode():
    ...
    a = int(a)
    b = int(b)
    cipher = input('Input your cipher text: ').lower()
    plain = ''
